...
vnoremap <silent> - :call RangeCommentLine()<CR>

" key-mappings for un-comment line in normal mode
noremap  <silent> _ :call UnCommentLine()<CR>
" key-mappings for range un-comment lines in visual <Shift-V> mode
vnoremap <silent> _ :call RangeUnCommentLine()<CR>
...
function! UnCommentLine()
  ...
  " for .tex use %
  elseif file_name =~ '\.tex$' || file_name =~ '\.nw$'
    execute ":silent! normal :nohlsearch\<CR>:s/%/\<CR>:nohlsearch\<CR>\<ESC>\<down>^"
  " for fortran 77 files use C on first column 
  elseif file_name =~ '\.f$' || file_name =~ '\.F$'
